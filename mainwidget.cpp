    ...
    if(0==xp.value(0).toInt())//数据库为空
    {
        ...
        connect(csh,&initGetMsg::initGetMsgOK,this,[=](QString cshUser,QString cshPwd,QString cshIdentity,QString filePath){
            if("团支书"==cshIdentity)
            {
                ...
                {
                    if(!axp.exec(QString("INSERT INTO %1 VALUES ('%2','%3','%4');").arg(cshUser).arg(cshIdList[i]).arg(cshNameList[i]).arg(cshEmailList[i])))
                    {
                        QMessageBox::information(this,"插入团支部成员信息提示","插入团支部成员信息失败:\n"+axp.lastError().text());
                        return;
                    }
                }
                axp.exec(QString("select *from %1;").arg(cshUser));
                while(axp.next()) qDebug()<<axp.value(0).toString()<<" "<<axp.value(1).toString()<<" "<<axp.value(2).toString();
            }
